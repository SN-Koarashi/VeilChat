...
function onKeyEnter(ele){	
	setTimeout(()=>{
		...
		/*
		$('.headerTitle').text($('.messageBox')[0].getBoundingClientRect().bottom + ','
			+ $('.messageBox')[0].getBoundingClientRect().top + ','
			+ $('.emoji-window')[0].getBoundingClientRect().bottom + ','
	...
	setTimeout(()=>{
		...
		var cHeight = (eachLine*21 > 150) ? 150 : eachLine*21;

		if(cHeight < 150 || $('.messageBox').height() < 150){
			...
			$(ele).addClass('scroll');
			if($(ele).prop('scrollHeight') - $(ele).scrollTop() == 194) $(ele).scrollTop($(ele).prop('scrollHeight'));
		}
...
function initFirst(window){
	...
	$('#sender').on('keydown',function(e){
		...
		if(e.keyCode == 9){
			...
			let textBefore = v.substring(0,  cursorPos);
			let textAfter  = v.substring(cursorPos, v.length);
			let insertLength = textBefore.length + 1;

			$('#sender').text(`${textBefore}　${textAfter}`);
		...
		if(e.keyCode == 13 && !e.shiftKey && !mobileAndTabletCheck() || e.keyCode == 9)
			e.preventDefault();
   ...
   $("body").on("touchmove", function(e) {
		...
		/*
		if(Math.abs(X) < 65 && Y > 100 && snkms.isShownDialog(e.target)){
			//console.log($(e.target).parents(".snkms-content").css('top'));
...
　　　　 //e.preventDefault();
    ...
    　　moveEndY = e.originalEvent.changedTouches[0].pageY;
		...
		if(!snkms.isShownDialog(e.target) && X != 0 && !touchScrolling && Math.abs(X) > Math.abs(Y) || $('.openBackground').hasClass('noOverflow')){
			...
			/*
			if (X > 15 && $(".rightSide").attr("data-open") && !touchStaying){
				toggleSidebar($(".rightSide"), false, "right");
			}
			else if(dragElement['.rightSide'] && $(".rightSide").attr("data-open")){
				if(parseFloat($('.openBackground').css('opacity')) > 0.5 || touchMovingDirection === -1){
					toggleSidebar($(".rightSide"), true, "right");
				}
				else{
					toggleSidebar($(".rightSide"), false, "right");
			...
			if (X < -15 && $(".wrapper_settings").attr("data-open") && !touchStaying){
				toggleSidebar($(".wrapper_settings"), false, "left");
				$("#userName").blur();
				savingSettings();
			}
			else if(dragElement['.wrapper_settings'] && $(".wrapper_settings").attr("data-open")){
				if(parseFloat($('.openBackground').css('opacity')) > 0.5 || touchMovingDirection === 1){
					toggleSidebar($(".wrapper_settings"), true, "left");
				}
				else{
...
　　　　 //e.preventDefault();
    ...
    　　moveEndY = e.originalEvent.changedTouches[0].pageY;
		...
		if(!touchScrolling && Math.abs(X) > Math.abs(Y) || touchMovingDirection !== 0 && !touchScrolling && parseInt($('.lobby').css('left')) !== 0){
			...
			/*
			if (X < -25 && !touchStaying){
				toggleSidebar($(".rightSide"), true, "right");
			}
			else if(X < 0 && dragElement['.rightSide']){
				if(parseFloat($('.openBackground').css('opacity')) > 0.5 || touchMovingDirection === -1 && !touchStaying){
					toggleSidebar($(".rightSide"), true, "right");
				}
				else{
					toggleSidebar($(".rightSide"), false, "right");
			...
			else if(X > 0 && dragElement['.wrapper_settings']){
				if(parseFloat($('.openBackground').css('opacity')) > 0.5 || touchMovingDirection === 1 && !touchStaying){
					toggleSidebar($(".wrapper_settings"), true, "left");
				}
				else{
	...
	$("body").on("click",function(e){
		if(e.target === downTarget){
			if(mobileAndTabletCheck() && $(".emoji-window").css('display') == 'block'){
	...
	$(".emoji-window .eBody .eContainer").on("click","div[data-id]",function(e){
		...
		$('#sender').text(`${textBefore}${beforeSpace}:${$(this).attr("data-id")}:${afterSpace}${textAfter}`);
