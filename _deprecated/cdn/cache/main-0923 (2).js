...
function ContentFormatter(text){
  ...
  var invitedText = parseInvite(urledText);
  
  return emojify(invitedText);
}
...
function parseInvite(text){
	...
	return text.replace(inviteRegex, function(matched, matchSub, offset, groups) {
		return '<a class="inviteLink" target="_self" href="https://chat.snkms.com/private/' + matchSub + '">邀請您加入 #私聊 ' + matchSub + '<span>加入</span></a>';
	});
}
